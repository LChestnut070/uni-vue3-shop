...
import {
  reqDeleteOrder,
  reqGetOrderById,
  reqGetOrderCancelById,
  reqGetOrderConsignmentById,
  reqGetOrderLogisticsById,
  reqOrderReceiptById,
} from '@/services/order'
...
// at(-1)代表数组的最后一项,指当前支付页
const curPagesInstance = curPages.at(-1) as any
onReady(() => {
  // 动画效果,导航栏背景色
  curPagesInstance.animate(
    '.navbar', // 选择器
  ...
  // 动画效果,导航栏标题
  curPagesInstance.animate('.navbar .title', [{ color: 'transparent' }, { color: '#000' }], 1000, {
  ...
  // 动画效果,导航栏返回按钮
  curPagesInstance.animate('.navbar .back', [{ color: '#fff' }, { color: '#000' }], 1000, {
    scrollSource: '#scroller',
    timeRange: 1000,
    startScrollOffset: 0,
    endScrollOffset: 50,
...
const getOrderById = async () => {
  const res = await reqGetOrderById(query.id)
...
const getOrderLogisticsById = async () => {
  const res = await reqGetOrderLogisticsById(query.id)
...
const onOrderPay = async () => {
  ...
  if (import.meta.env.DEV) {
    // 如果为开发环境
    await reqGetPayMock({ orderId: query.id })
    uni.hideLoading()
  } else {
    // 如果为生产环境
    const res = await reqGetWxPayMiniPay({ orderId: query.id })
    uni.hideLoading()
...
const onOrderSend = async () => {
  if (isDev) {
    await reqGetOrderConsignmentById(query.id)
...
const onOrderConfirm = () => {
  ...
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        const res = await reqOrderReceiptById(query.id)
        // 更新订单状态
        order.value = res.result
      }
    },
...
const onOrderCancel = async () => {
  await reqGetOrderCancelById(query.id, { cancelReason: reason.value })
...
const onOrderDelete = async () => {
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        await reqDeleteOrder({ ids: [query.id] })
