...
import {
  reqGetOrderPreByGoods,
  reqGetOrderPreByCart,
  reqSubmitOrder,
  reqGetOrderRepurchaseById,
} from '@/services/order'
...
// 当前配送时间
const activeDelivery = ref<typeof deliveryList.value[0]>(deliveryList.value[0])
const onChangeDelivery: UniHelper.SelectorPickerOnChange = (ev) => {
  // 下标
  activeDelivery.value = deliveryList.value[ev.detail.value]
}
...
const getOrderPre = async () => {
  if (query.count && query.skuId) {
    // 如果从商品详情页进入
    const res = await reqGetOrderPreByGoods({
      count: query.count,
      skuId: query.skuId,
    })
    ...
    // 如果是从再次购买进入
    const res = await reqGetOrderRepurchaseById(query.orderId)
    orderPre.value = res.result
  } else {
    // 如果从购物车进入(不用传递参数,因为后端会默认将购物车已经选中的商品作为已下单的商品)
    const res = await reqGetOrderPreByCart()
    orderPre.value = res.result
  }
}
