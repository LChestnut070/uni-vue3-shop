...
import { reqGetMemberProfile, reqPutMemberProfile } from '@/services/profile'
...
// 获取会员信息
/* 修改信息时需要提供初始值,否则v-model会报错 */
const profile = ref<ProfileDetail>({} as ProfileDetail)
const getMemberProfile = async () => {
  const res = await reqGetMemberProfile()
...
const onSave = async () => {
  ...
  if (changeFlag) {
    // 省会编码
    params.provinceCode = fullLocationCode[0]
    // 城市编码
    params.cityCode = fullLocationCode[1]
    // 区县编码
    params.countyCode = fullLocationCode[2]
  }
  // 发请求
  const res = await reqPutMemberProfile(params)
  // 同步显示昵称
