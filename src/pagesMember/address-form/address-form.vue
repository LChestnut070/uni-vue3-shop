...
import { reqAddAddress, reqGetAddressInfoById, reqUpdateAddress } from '@/services/address'
...
// 更改地区
const onLocationChange: UniHelper.RegionPickerOnChange = (e) => {
...
const onSubmit = async () => {
  try {
    ...
    if (query.id) {
      // 修改地址
      await reqUpdateAddress(query.id, form.value)
    } else {
      // 新增地址
      await reqAddAddress(form.value)
    }
    uni.showToast({
      icon: 'success',
...
const getAddressInfoById = async () => {
  if (query.id) {
    const res = await reqGetAddressInfoById(query.id)
